...
- **External-repo landscape (2026-07-13 deep study, `docs/research/STUDY-raganything-agentscope-openspace.md`):** HKUDS/OpenSpace identified as MDLP's **closest living relative** — a self-evolving skill layer (MCP + SKILL.md, experience counters, LLM-gated skill evolution, lineage DAG) that is a point-estimate, statistics-free degenerate of MDLP's design; natural paper baseline. RAG-Anything/LightRAG = §16/graph-store build patterns (retrieval modes, fusion ranking, set-merge + content-hash dedup); AgentScope 2.0 = `mdlp` library engineering patterns. All 7 proposals EXECUTED 2026-07-13: 4 gated changes APPROVED through review-360→change-approver (R1 §16-companion 87/3rds · §17.6 lineage schema 84/7rds · B2 Amendment A 85/8rds · DATA-LAYER §6.1/6.2 write discipline 83/12rds — decision records in `docs/research/reviews/`), plus ungated `M1-EVAL-PROTOCOL.md`, IMPLEMENTATION-v2 §20, LANDSCAPE §4.4a + PAPER §2 positioning. All working-tree only — commits await user authorization. [verified]
