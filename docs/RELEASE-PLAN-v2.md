...
> **STATUS UPDATE — 2026-08-13 (owner decision: the 2026-08 research harvest is IN v2 scope).** `docs/research/PLAN-harvest-2026-08.md` (six work packages from the Raft/ontology/abduction/prospection arc) is adopted into this release as follows — mapping amendable, adoption not:
> - **Ships as gated spec deltas before the v2 tag:** WP-A's spec-side parts (RAF-2 consolidations) · **WP-B the invariant layer** — RAF-1a's safety-properties figure (ALGORITHM §21) is the flagship v2 spec addition and the first gate submission, with the RAF-1b truth-replay checker spec'd in DATA-LAYER, ONT-1′'s constraint table generalizing the approved acyclicity check, and RAF-3 epochs · **WP-D** learning liveness (one §20.6/§20.7 delta).
> - **Ships as pre-registered protocol:** WP-C — ONT-4 competency questions + the RAF-4 implementability protocol join `M1-EVAL-PROTOCOL.md` as release deliverables; the spec-as-corpus run is the bridge evidence for G4 and the paper's Ousterhout-style study.
> - **Ships as documented design (Tier ③, labelled):** WP-E beyond ONT-7 (JMP-2 invention engine, VIS-1/2 prospection — the studies/notes ship as design, implementation post-v2) · peer learning (M-R revisit).
> - **Paper-side (WP-F):** JMP-3's scope statement (Peirce naming; evidence-free restructuring out of scope, Zahavy 2026 as boundary), the axiom-layer contribution position, IX-040 as the credibility hook, ONT-5 skill-identity mappings.
> - **WP-A's framework-side parts (RAF-7's three gate devices) land as `evolve:` commits immediately — they are the authoring quality bar for every other item above.**
> - **New owner decision [D-4]:** does the spec-compression middle path (spec-of-record ~12 sections + appendix-of-record) execute **before** the v2 tag (reviewers see the compressed spec) or **after** (v2 ships the current 20 sections + the property figure)? Recommendation: *before* — the property figure plus a compressed reading path is the strongest possible reviewer-facing artifact — but it is the larger pre-tag effort.
>
