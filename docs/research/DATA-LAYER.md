...
MergeReport{ added: [id], deduped: [id], merged: [(id,id)], retired: [id], updated: [id], rejected: [(id, reason)],
             flags: [(id, constraint_id, severity)] }   # §6.3 delta — constraint hits on admitted entries
```
...
### 6.3 The structural constraint table — declared admissibility at `merge()` *(added 2026-08-13 — IN GATE)*

*Generalizes what §6.2 already does once: MERGE enforces exactly one structural constraint today — B2 Amendment A's union-scoped acyclicity, hard-coded, reporting via `MergeReport.rejected` — plus the liveness-shape check (`test_merge_never_assigns_liveness`). §6.3 turns "a check someone remembered to code" into **a declared table the write path evaluates**, so the constraint inventory is data with an audit trail, not scattered code paths discovered one review round at a time (the reactive-constraint pattern, L-018; third sighting documented in `STUDY-ontologies-for-mdlp.md` §2). **Property-impact statement (§21.3), per-property: PR-6 strengthened** (the enforcement PR-6 rests on is generalized at the only projection write path, §6.2); **PR-1–PR-5, PR-7–PR-9 preserved** (no property statement, gate, or update path touched). Schema additions: two administrative event kinds (`constraint_flag`, `constraint_table` — §5, §6.1 exemption list). Provenance: SHACL's closed-world validation discipline (severities, reject-and-report, declared shapes) with **no RDF anywhere** — the table is a stdlib structure over `GraphDelta`, identical on both tiers.*

**Gate ordering, stated once (the cost argument):** admission to the live graph is three gates in cost order — **Gate 0** shape/type (dataclass construction, already present) → **Gate 1** structural admissibility (this table — deterministic, O(|delta|) plus the §6.2-costed acyclicity check, free relative to eval) → **Gate 2** statistical (§8, a held-out eval budget). A structurally illegal candidate today can burn Gate-2 budget before anything refuses it; Gate 1 refuses it at the door.

**Evaluation semantics — reject-and-report, never infer (the OWL lesson, made a write-path rule):**
- Constraints are evaluated inside `merge()`, between delta receipt and application. **Evaluation scope by target:** `node`-target entries evaluate over `delta.adds`, `edge`-target over `delta.edges` and `delta.edge_updates`, `delta`-target over the whole delta (including post-application predicates like acyclicity). `table_version` is assigned atomically in the registration append's own transaction — the §6.1 `seq` pattern, no second counter mechanism. **No constraint outcome ever resolves an inconsistency by inferring, coercing, merging, or silently fixing** — the write path rejects and reports, full stop. (Open-world inference at a write boundary is how a violation becomes a silent graph rewrite — the failure `STUDY-ontologies-and-raft.md` §2 documents in OWL and RC-4 documents in MDLP's own `coherent_with` absorption.)
- **Severities, mapped to existing machinery — MERGE stays a projection writer, never a gate:**
  - `violation` → the entry is **rejected**: `MergeReport.rejected += (id, constraint_id)` — the existing per-entry semantics of the acyclicity and liveness checks, unchanged; the rest of the delta applies under §6.2's transaction contract. **Attribution for `delta`-target constraints:** the rejected unit is each entry whose application violates the predicate — for acyclicity, each cycle-closing edge (the existing B2-AmendA per-edge semantics: "an authored `part_of` edge closing a cross-type cycle is rejected at insert"); for per-entry quantified predicates (e.g. `C-RETIRE-PROVENANCE`), each offending entry, one flag each. Where no single entry is attributable, the whole delta rejects with one flag — never a silent partial guess.
  - `warning` → the entry is **admitted unchanged** — a warning **never** alters a status or payload (altering status would make MERGE assign liveness, which §6.2 forbids) — and the flag is reported in the **ordinary §20.7 digest (suppressible)**: warnings alter nothing, and the always-delivered class stays reserved for the conditions ALGORITHM's §20.7/§20.10 enumerate (the §20.10 structure-freeze precedent — informative, not alarm-class).
  - `info` → admitted, recorded, nothing surfaced.
  - **The channel, precisely (who reports, who records):** `MergeReport` gains one field — `flags: [(id, constraint_id, severity)]` — carrying every hit of every severity synchronously to the caller (rejections additionally appear in `rejected`, unchanged). The **§6 orchestrator** (JUDGE-side, already the component that writes truth) then emits one `constraint_flag` administrative event (§5) per flag, referencing the report — `GraphStore.merge()` itself never touches TruthStore (stores stay single-purpose; the §17.6 pattern: the orchestrator observes and records). `test_every_hit_flagged` asserts the orchestrator's emission, not a store side-effect. **Durability of the emission (the crash window, closed the §6.1 way):** `constraint_flag` ids are deterministic — `hash("constraint_flag" ‖ merge_report_ref ‖ constraint_id ‖ target_id)` — and the merge report itself is archived with the cycle outcome (§20.7(a)), so the recovery scan re-derives and re-emits any flags lost between `merge()` returning and the appends landing; re-emission dedupes by identity (`test_flag_emission_crash_recoverable`). A flag can be delayed by a crash, never silently lost.
- **The predicate vocabulary is fixed and small** (decidable, stdlib-evaluable, no user code execution — the same no-`eval` reasoning as §17.1's wall check): *cardinality* (field counts, fan-in/fan-out caps) · *value-type/range* (enum membership, `[0,1]` bounds) · *closed shape* (no fields beyond the schema's — how a delta carrying an undeclared field is caught) · *property-pair* (cross-field requirements) · *edge-set acyclicity* (over declared type unions) · *namespace* (reserved-name rules). A constraint needing logic outside this vocabulary is code, belongs in §6.2's contract or the algorithm layer, and is out of this table's scope by construction.

**The table itself:**
```
ConstraintEntry{ constraint_id, target ∈ node|edge|delta,
                 predicate: [(family, params), ...],    # a CONJUNCTION of atoms from the fixed
                                                        # vocabulary — all must hold; an entry
                                                        # combining families (e.g. enum + non-null
                                                        # pair) is a list, expressible as written
                 severity ∈ violation|warning|info,
                 anchor,                                 # approved-text §-ref (violation-grade: required)
                 fire_test }                             # the §20.6-discipline test that violates it
```
- **Registered as JUDGE data, versioned in truth:** a `constraint_table` event (§5) carries the full table inline (`content_hash` for integrity, the §12 `conformance_manifest` pattern). SOLVE has no write path to it (it is JUDGE configuration, §17.1). Registration is an orchestrator-side admission append (the §17.6 pattern); its mechanical check is **presence and format** of `authorized_by` for gated versions (below) — *validity* of the cited decision record is auditable after the fact (§12-checker territory), stated honestly rather than claimed as a runtime verification.
- **Amendment rule — one set-based test decides what is gated:** a new table version **requires the L-010 gate iff its `violation`-grade entry set differs from its predecessor's in any way** — entry added, removed, upgraded to violation, downgraded from violation, or its predicate/params changed. For such versions `authorized_by` cites the authorizing decision record. All other version changes (adding, editing, or removing `warning`/`info` entries — which alter no admissibility) are curator/operator-grade: gate-free, with `authorized_by` carrying the registering actor and a rationale string instead of a decision record. **Anchor rule, resolved by severity:** `violation`-grade entries always require an approved-text anchor (they enforce norms, and norms come from gated text); `warning`/`info` entries may be **unanchored, explicitly marked `observational`** — they impose nothing, so they may watch for patterns no approved text yet names.
- **Fire-test discipline:** every entry ships with a test that feeds `merge()` a violating delta and asserts the flag/rejection (`test_constraint_fires[constraint_id]`) — a constraint that cannot be made to fire is the inert-alarm failure (§12.4's rule, applied to the table). The §12 conformance manifest carries one always-fires entry for the table itself (constraint evaluation runs on every grow-tick merge).

**Initial registered set — approved-grounded entries only.** Each entry cites the approved text it operationalizes; **no new norm is introduced by this table** (an entry without an approved-text anchor does not ship in the initial set — adding one is an amendment, above):

| id | target | predicate | severity | anchor |
|---|---|---|---|---|
| `C-ACYCLIC` | delta | acyclicity over `prereq ∪ part_of` after application | violation | B2 Amendment A (BUILD-SPECS) — **migrated**: rejection-set-identical to the existing hard-coded check, now table-driven; B2-AmendA's text ("enforced at the graph write path: `GraphStore.merge()` validation, reporting via `MergeReport.rejected`") remains accurate as written, so no BUILD-SPECS amendment accompanies this |
| `C-LIVENESS-SHAPE` | node | status ∈ {live, pending_human}; `live` ⇒ non-null `suite_ref` *(the "pre-decided" requirement itself is §6.2's EXTRACT rule, enforced there — this entry checks the resulting shape, which is what a structural table can check)* | violation | §6.2 EXTRACT rule + RC-3 (`provision_suite` invariant); the existing `test_merge_never_assigns_liveness`, table-driven |
| `C-EDGE-TYPE` | edge | type ∈ {prereq, part_of, transition} with the B2-AmendA field set (`weight`, `confidence`, `hard` for typed hierarchy edges); closed shape — no undeclared fields | violation | DL §5 edge schemas; B2 Amendment A |
| `C-CONF-RANGE` | edge | `confidence ∈ [0,1]` | violation | §5.2 / B2-AmendA (`confidence` is a belief; out-of-range is malformed, not merely wrong) |
| `C-FANIN-CAP` | node | prereq fan-in ≤ the §5.1 `add_soft_prereq_edges` top-k cap *(the cap exists in approved text as that function's k but is **unregistered in §12 — an ALGORITHM-side registration gap, flagged here and out of this delta's scope**, the §12-checker-PR-9 pattern; until registered, `predicate.params` names the dial as `§5.1.top_k` and binds to its configured value — parameter-by-name, never a literal; retuning it is the ALGORITHM's governance, not a table amendment)* | violation | §5.1 `add_soft_prereq_edges` ("soft, capped fan-in, acyclic") |
| `C-NAMESPACE` | node | no suite-bearing node claims the reserved `maintenance` namespace | violation | ALGORITHM §20.2 (`provision_suite` rejects it; this entry closes the same door at the projection write path — defense in depth at the second write surface, same norm) |
| `C-RETIRE-PROVENANCE` | delta | non-empty `retires` ⇒ non-empty `delta.provenance` *(the schema's provenance is delta-level — `[truth_event_ids]` — so the honest checkable predicate is delta-level too; per-retire attribution would require a schema §6.2 does not have, and this entry does not pretend otherwise)* | warning | §6.2 `GraphDelta` schema (`provenance` is a stated delta field; a retire batch with no traceable provenance is reviewable, not illegal) |

*Table notes: every initial entry's `fire_test` is `test_constraint_fires[constraint_id]` (the column is omitted above because the binding is uniform). The initial set contains no `info` entry — the severity exists for future observational use and is exercised by its fire-test harness, not by shipped entries. "Migrated" for `C-ACYCLIC` means **rejection-set-identical** (same deltas rejected — `test_migrated_acyclicity_identical`'s exact assertion); the `MergeReport.rejected` reason string becomes `C-ACYCLIC`, which is the one visible difference.*

**Port delta, declared:** `MergeReport` gains one additive field — `flags: [(id, constraint_id, severity)]` (the synchronous channel for warning/info hits on admitted entries; rejections continue to use `rejected` with `constraint_id` as the reason string). `merge()`'s signature is otherwise unchanged and gains no store dependency (event emission is the orchestrator's, above). `schemas.py` gains `ConstraintEntry` and the two §5 event dataclasses.

**Tests (§6.3 additions to the write-discipline set):** `test_constraint_fires[constraint_id]` (one per entry, violating-input) · `test_constraint_table_versioned_in_truth` (evaluation uses the highest `constraint_table` version; the report cites it) · `test_violation_rejects_entry_not_delta` (a violating entry rejects per-entry; sibling entries apply — matching existing acyclicity semantics) · `test_warning_admits_unchanged` (a warned entry's status/payload is byte-identical to its EXTRACT form — MERGE never gates) · `test_every_hit_flagged` (each constraint hit of any severity has a `constraint_flag` event referencing the merge report) · `test_no_constraint_coerces` (no merge output differs from its input except by rejection — the reject-and-report rule as a property) · `test_migrated_acyclicity_identical` (C-ACYCLIC's rejections are exactly the pre-table hard-coded check's rejections on the same deltas) · `test_violation_amendment_requires_authorization` (a `constraint_table` version changing a violation-grade entry without `authorized_by` citing a decision record is refused at registration).
