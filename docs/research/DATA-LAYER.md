...
A `Stores` bundle (a dataclass holding one instance of each port) is what the algorithm receives. **Two bundles exist** (§6.1 P1 boundary): the JUDGE-side bundle holds the unredacted `TruthStore`; the SOLVE-side bundle holds a `RedactedTruthView` (same interface; held-out `item_ids`/content stripped) — wired at `build_stores()` time, outside every SOLVE-editable surface.
