...
### 4.4a Shipped self-evolving skill layers — OpenSpace (the closest *running* relative)  [E — verified from source, 2026-07-13]

**OpenSpace** (HKUDS, MIT, v0.1.0, open-sourced 2026-03) is a **self-evolving skill layer** for LLM agents: an MCP server whose `SKILL.md` skills are selected by hybrid BM25+embedding retrieval, monitored by four per-skill experience counters (`selections/applied/completions/fallbacks`), and **evolved** (FIX / DERIVED / CAPTURED operators) when point-estimate rates cross **hardcoded thresholds**, each trigger confirmed by an **LLM judgment call** before mutation; full lineage DAG with snapshots/diffs and reactivation-rollback; cloud skill sharing **without receiver-side re-validation**. Its GDPVal two-phase benchmark (cold accumulation → warm rerun, fixed backbone) reports 4.2× task income and 45.9% token cost *(authors' numbers — self-reported)*. **What it lacks is exactly MDLP's core:** no posterior variance, no significance tests, no held-out gating, no calibration, no curriculum (it evolves what breaks; it never chooses what to learn next), and zero automated tests. Deep-dive with file-line citations: `STUDY-raganything-agentscope-openspace.md` §3. **[E** mechanisms, from source **/ P** benchmark numbers**]**

**Hermes Agent** (Nous Research, MIT, v0.19; verified from source 2026-07-28) is the production-hardened sibling: skills created/patched by an LLM background fork on a dumb iteration counter, memory nudges, FTS5 session-search-as-memory, a two-speed curator — and **no outcome measurement anywhere**: its usage telemetry records occurrence, not success, and its own curator prompt instructs the model to distrust the counters ("use=0 … is absence of evidence"). It also ships a trajectory-generation + student-tokenizer-budgeted compression pipeline for tool-calling SFT — the "curriculum → fine-tune data engine" shape with a boolean `completed` where MDLP puts statistical gates. Deep-dive: `STUDY-hermes-agent.md`. With OpenSpace, automaton, and hermes all verified from source, the pattern is three-for-three: **no shipped self-improving agent measures whether its learning worked.** **[E** from source**]**

**Rox** (closed-source, Sequoia-backed "Revenue Operating System"; vendor engineering docs read 2026-08-19 — a class below source-verified) extends the pattern to the **commercial, funded, multi-tenant tier**: a production agent swarm with genuinely strong evaluation engineering (sandboxed ~200-query suite over mocked service contracts, per-module offline/online evaluator pairs, versioned module I/O replayed against historical production data, evals on tool calls and pipeline shapes) — and **no learning loop, by its own written admission**: its evaluation-system doc concedes current "LLM systems are stagnant — they're built once," files feedback capture and "dynamic adjustments that keep agents evolving" as future direction, and separately lists "proactivity, adaptability, and memory" as unbuilt. User-side "tuning" is configuration checkboxes; per-item approve/reject gates distribution, trains nothing. This upgrades the evidence class: the first three absences were *inferred from source*; Rox's is an **admission against interest in vendor documentation**. Four production systems examined, four without learning measurement. Deep-dive: `STUDY-rox-agent-swarm.md`. **[P** mechanisms, vendor-described; **E**-grade for the admission itself**]**
