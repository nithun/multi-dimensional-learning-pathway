...
**Self-evolving agents and managed-agent platforms.** Meta-agents that design agents (ADAS; Hu et al., 2024) and surveys of self-evolving agents (e.g., Fang et al., 2025) establish the mechanisms but as research artifacts, not platforms. Conversely, managed-agent control planes orchestrate agents as teammates without a persistent learning loop. The closest shipped systems are OpenSpace (HKUDS, 2026), a self-evolving skill layer whose skills carry per-skill success/attempt counters and are mutated (fix/derive/capture operators, full lineage with rollback) when point-estimate rates cross fixed thresholds, each mutation confirmed by an LLM judgment; and Hermes Agent (Nous Research, 2026), whose skills are created and patched by an LLM reviewer fired from an iteration counter, with usage telemetry that records occurrence but never outcome — its own curation prompt instructs the model to disregard the counters as evidence. It demonstrates demand and a working end-to-end loop, but its competence estimates carry no uncertainty, its triggers are hand-tuned constants, and no held-out measurement separates real improvement from noise — precisely the statistical machinery this framework supplies (calibrated dual posteriors, significance-gated commits, measurement independence). The white space — a generic learning loop, spanning both memory and parameter improvement and sharing one evaluation signal, attached to a live agent fleet — remains, to our knowledge, unoccupied; this framework targets it, and OpenSpace-style skill layers are its natural first deployment surface and comparison baseline.
...
## 5. Evaluation protocol (pre-registered; design only, with one executed milestone)

We present the protocol that will test the framework. Success criteria and analyses were fixed in advance — before any result existed — to prevent the measurement-dependence (P1) the framework itself warns against. The protocol is staged to the implementation milestones (M0→M2). One milestone (M0, agent instantiation) has since been executed under the pre-registered criteria; its result is reported in §5.9 and nowhere shaped the protocol.
...
### 5.9 First executed milestone: M0 on the agent instantiation (2026-07-01)

The M0 gate was run once, under the pre-registered criteria of §5.7, on the agent instantiation of the reference implementation. **Result: GO.** With a live production LLM in the loop (920 model calls, zero runner errors) on a held-out split of an unseen synthetic-cipher skill (a knowledge-bottleneck task; $n_{\text{held}}=15$, 10 learning ticks, paired gating at $z=2$): held-out competence reached **0.495** under the learning loop versus **0.025** under the frozen no-learning baseline — a margin of **+0.47** — and the memorization/hard-coding probes failed as designed (H1 holds; H6's M0 probes held). Selection ran through the learning-gain policy of §3 (Thompson sampling over learning progress with the coverage floor), not a fixed rotation. A prior adversarial review of the implementation preceded the run and corrected two defects that would have silently nulled two components; two earlier runs invalidated by an infrastructure fault were discarded and are not counted.

**Scope and honesty.** This is a single skill, in a single run, in one domain, on a task family chosen for verifier reliability rather than representativeness; it establishes existence (the loop moves true held-out competence beyond a no-learning baseline under P1 conditions), not effect-size generality. H2 (objective comparison), the M1 growth hypotheses at powered sample sizes, and a representative code+test corpus remain future executions of this protocol. Milestone-1 mechanisms (growth provisioning, spacing, warm-start, prerequisite-gap diagnosis, revisit triggers) have additionally been confirmed to *operate* end-to-end live — wiring validation at small $n$, which we explicitly do not report as powered margins.
